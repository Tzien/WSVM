          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="record.rowEdit">
              ...
              <template v-else-if="['select', 'radio', 'checkbox'].includes(column.ceriKey)">
                <jnpf-select v-model:value="record[column.prop]" :placeholder="column.placeholder" :multiple="column.multiple || column.ceriKey === 'checkbox'" :allowClear="column.clearable || ['radio', 'checkbox'].includes(column.ceriKey)" :showSearch="column.filterable" :disabled="column.disabled" :options="column.options" :fieldNames="column.props" />
			  </template>
...
<script lang="ts" setup>
  ...
  const { childColumnList, searchSchemas, viewList, currentView  } = toRefs(state);
    ...
    moduleId:'afceac99-44de-4ae6-aa45-b4adef2cd647',
    superQueryJson: '',
  };
