...
<template v-if="!(record.top || column.id?.includes('-'))">
            ...
            <template v-if="column.ceriKey === 'rate'">
                <ceri-rate :value="formatNumberCell(record[column.dataIndex])" :count="column.count" :allowHalf="column.allowHalf" disabled />
            </template>
            <template v-if="column.ceriKey === 'slider'">
                <ceri-slider :value="formatNumberCell(record[column.dataIndex])" :min="column.min" :max="column.max" :step="column.step" disabled />
            </template>
...
<script lang="ts" setup>
  ...
  function formatNumberCell(val: any) {
  ...
  const defaultSearchInfo = {
    ...
    //moduleId:'641e151a-39ed-446a-a4ca-a37faae835e3',
    superQueryJson: '',
  };
  ...
  const [registerTable, { reload, setLoading, getFetchParams,getSelectRows, getSelectRowKeys, redoHeight, clearSelectedRowKeys }] = useTable({
    ...
    afterFetch: (data) => {
      ...
      const mapRow = (row: any) => {
        ...
        state.columnList
          .filter((column: any) => ['rate', 'slider'].includes(column?.ceriKey))
          .forEach((column: any) => {
            if (!column?.prop) return;
            mapped[column.prop] = formatNumberCell(mapped[column.prop]);
            const pascalKey = column.prop.replace(/^[a-z]/, (s) => s.toUpperCase());
            if (pascalKey in mapped) mapped[pascalKey] = formatNumberCell(mapped[pascalKey]);
          });
        return mapped;
      };
