...
<script lang="ts" setup>
  ...
  function getColumnFieldKey(key: string) {
    const columns = state.columnList?.length ? state.columnList : columnList;
    const matchedKey = (columns || []).map((item) => item?.prop).find((item) => item && item.toLowerCase() === key.toLowerCase());
  ...
  const [ registerTable, { reload, setLoading, getFetchParams, getSelectRows, getSelectRowKeys, redoHeight, insertTableDataRecord, updateTableDataRecord, deleteTableDataRecord, clearSelectedRowKeys }] = useTable({
    ...
    afterFetch: data => {
      const rows = (Array.isArray(data) ? data : data?.list || []).map(o => {
        const mapped: any = { ...o };
        Object.keys(o || {}).forEach(key => {
          const fieldKey = getColumnFieldKey(key);
          if (!(fieldKey in mapped)) mapped[fieldKey] = o[key];
        });
        const rowId = mapped.id ?? mapped.Id;
