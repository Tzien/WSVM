    ...
    <a-row class="dynamic-form ">
      <a-form :colon="false" layout="horizontal" labelAlign="right"  :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formRef">
        <a-row :gutter="15">
          ...
          <a-col :span="24" class="ant-col-item" >
            <a-form-item name="ModifyTime" :labelCol="{ style: { width: '100px' } }">
              <template #label>时间选择</template>
              <CeriTimePicker v-model:value="dataForm.ModifyTime" placeholder='请选择' format="HH:mm:ss" allowClear :style='{"width":"100%"}'  />
            </a-form-item>
          </a-col>
          <a-col :span="24" class="ant-col-item" >
            <a-form-item name="YanSe" :labelCol="{ style: { width: '100px' } }">
              <template #label>颜色选择</template>
              <CeriColorPicker v-model:value="dataForm.YanSe"  />
            </a-form-item>
          </a-col>
          <a-col :span="24" class="ant-col-item" >
...
function getData(id) {
  getInfo(id).then((res) => {
    ...
    if (mapped.ModifyTime) mapped.ModifyTime = formatTimeFormValue(mapped.ModifyTime);
  ...
  function buildSubmitData() {
    ...
    if (data.ModifyTime || data.ModifyTime === 0) data.ModifyTime = formatTimeSubmitValue(data.ModifyTime);
