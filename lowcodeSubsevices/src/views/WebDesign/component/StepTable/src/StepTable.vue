<template>
  <div :class="`${prefixCls}`">
    <div style="height: 100%" v-show="formInfo.webType == 2 || formInfo.webType == 4">
    ...
    <div class="column-empty-box" v-show="formInfo.webType == 1">
      ...
      <p>开启后，表单带有数据列表</p>
      <a-button type="primary" style="width: 150px" size="large" @click="toggleWebType">开启列表</a-button>
    </div>
  </div>
</template>
...
const getBindValue = computed(() => {
  return { 
    ...omit(props, ['columnData', 'appColumnData']), 
    conf: props.columnData 
  };
});
...
function getData() {
  return new Promise((resolve, reject) => {
    ...
    const appColumnData = { columnList: [] }
