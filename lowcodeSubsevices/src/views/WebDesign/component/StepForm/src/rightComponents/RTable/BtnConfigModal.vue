    ...
    <a-form
      ...
      <template v-if="!['copy', 'remove'].includes(dataForm.value)">
        ...
        <a-form-item label="按钮图标">
          <ceri-icon-picker v-model:value="dataForm.btnIcon" placeholder="请选择" />
        </a-form-item>
      </template>
      ...
      <template v-if="!['copy', 'remove', 'add', 'batchRemove'].includes(dataForm.value)">
        <ceri-group-title content="动作设置" :bordered="false" />
        ...
        <template v-else>
          ...
          <a-form-item label="弹窗类型">
            <ceri-radio v-model:value="actionConfig.popupType" :options="popupTypeOptions" optionType="button" button-style="solid" />
          </a-form-item>
          ...
          <a-form-item label="表单数据" v-else>
            <CeriTreeSelect v-model:value="actionConfig.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
          </a-form-item>
          ...
          <a-table :data-source="actionConfig.columnOptions" :columns="getColumnOptionsColumns" size="small" :pagination="false">
            ...
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.key === 'value'">
                ...
                <ceri-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch @change="onChange($event, record)" />
              </template>
          ...
          <a-table :data-source="actionConfig.relationOptions" :columns="relationOptionsColumns" size="small" :pagination="false">
            <template #bodyCell="{ column, record, index }">
              <template v-if="column.key === 'field'">
                <ceri-select v-model:value="record.field" placeholder="请选择" :options="childList" showSearch class="!w-134px" />
              </template>
              <template v-if="column.key === 'type'">
                <ceri-select v-model:value="record.type" placeholder="请选择" :options="typeOptions" @change="onTypeChange(record)" />
              </template>
              <template v-if="column.key === 'value'">
                <template v-if="record.type == 1">
                  <a-auto-complete
                    ...
                    @focus="onFocus(record.value)"
                    @search="debounceOnSearch(record.value)" />
                  <ceri-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch />
                </template>
...
<script lang="ts" setup>
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowList.includes(data.__config__.cerikey))
        list.push({
          id: data.__vModel__,
  ...
  function init(data) {
    ...
    childList.value = cloneDeep(data.children)
      .filter(o => o.__vModel__ && !noAllowSelectList.includes(o.__config__.cerikey))
      .map(o => ({
        id: o.__vModel__,
  ...
  function onRelationFieldChange(val, row) {
    if (!val) return (row.cerikey = '');
    let list = unref(formFieldsOptions).filter(o => o.__vModel__ === val);
    if (!list.length) return (row.cerikey = '');
    let item = list[0];
    row.cerikey = item.__config__.cerikey;
  }
