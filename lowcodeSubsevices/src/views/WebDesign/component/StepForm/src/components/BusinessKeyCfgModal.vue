  ...
  <BasicModal v-bind="$attrs" @register="registerModal" title="规则设置" @ok="handleSubmit" :minHeight="100">
    <a-form :colon="false" :labelCol="{ style: { width: '80px' } }" :model="dataForm" :rules="formRules" ref="formElRef">
      ...
      <a-form-item label="联合字段" name="businessKeyList">
        <ceri-select v-model:value="dataForm.businessKeyList" :options="options" multiple showSearch />
      </a-form-item>
      <a-form-item label="提示语" name="businessKeyTip">
        <ceri-input v-model:value="dataForm.businessKeyTip" placeholder="请输入" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  function getOptions(drawingList) {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && allowList.includes(data.__config__.ceriKey)) {
        list.push({
          id: data.__vModel__,
    ...
    loop(drawingList);
    list = list.filter(o => o.id.indexOf('_ceri_') < 0);
    state.options = list;
  }
