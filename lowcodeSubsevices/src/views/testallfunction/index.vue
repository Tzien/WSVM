  ...
  function setListValue(data: any[] = [], defaultData: any[] = [], key) {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < defaultData.length; j++) {
        if (data[i].show && sameId(data[i].id, defaultData[j][key])) list.push(defaultData[j]);
      }
    }
    // 视图配置中没有记录的列（如新增字段）默认显示
    for (let j = 0; j < defaultData.length; j++) {
      if (!data.some((o) => sameId(o.id, defaultData[j][key]))) list.push(defaultData[j]);
    }
    return list;
  }
  onMounted(() => {
