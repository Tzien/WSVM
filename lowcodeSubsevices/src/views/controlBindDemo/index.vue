...
<template v-if="!(record.top || column.id?.includes('-'))">
            ...
            <template v-if="column.ceriKey === 'colorPicker'">
                <div class="color-cell" :title="record[column.dataIndex]"
                     :style="{ width: '20px', height: '20px', margin: '0 auto', border: '1px solid #e5e7eb', backgroundColor: record[column.dataIndex] || 'transparent' }"></div>
            </template>
            <template v-if="column.ceriKey === 'editor'">
                <div class="rich-text-cell" v-html="record[column.dataIndex]"></div>
            </template>
            ...
            <template v-if="column.ceriKey === 'timePicker'">
                {{ formatTimeCell(record[column.dataIndex]) }}
            </template>
</template>
...
<script lang="ts" setup>
  ...
  import { thousandsFormat,getParamList } from '@/utils/ceri';
  ...
  function formatTimeCell(val: any) {
    ...
    const n = Number(val);
    const d = (!isNaN(n) && typeof val !== 'string') ? dayjs(n) : dayjs(val);
    return d.isValid() ? d.format('HH:mm:ss') : String(val);
  }
  ...
  const defaultSearchInfo = {
    ...
    //moduleId:'7f05a6af-a3d2-42d9-b2d4-0def749afcc3',
    superQueryJson: '',
  };
...
function addHandle() {
  ...
  // 确保富文本列存在
  if (!columns.some((o) => o.prop === 'rich_text_content')) {
    columns.push({
      label: '富文本',
      title: '富文本',
      prop: 'rich_text_content',
      dataIndex: 'rich_text_content',
      align: 'left',
      fixed: false,
      ceriKey: 'editor',
      sortable: false,
  ...
  // 确保颜色列存在
  if (!columns.some((o) => o.prop === 'color_value')) {
    columns.push({
      label: '颜色',
      title: '颜色',
      prop: 'color_value',
...
function initViewList(currentId = '') {
    ...
    getViewList(query).then(res => {
      ...
      // 确保当前视图包含富文本列
      try {
        const cv: any = state.currentView;
        if (cv && Array.isArray(cv.columnList)) {
          const exists = cv.columnList.some((it: any) => it && it.id === 'rich_text_content');
          if (!exists) {
            cv.columnList.push({ label: '富文本', id: 'rich_text_content', show: true, fixed: 'none' });
          }
        }
      } catch {}
    });
