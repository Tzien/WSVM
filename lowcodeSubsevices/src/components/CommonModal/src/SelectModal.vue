        ...
        <div class="ceri-common-search-box ceri-common-search-box-modal">
          ...
          <div class="ceri-common-search-box-right">
            <a-tooltip placement="top">
              ...
              <RedoOutlined class="ceri-common-search-box-right-icon" @click="initData" />
            </a-tooltip>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  function handleSubmit() {
    ...
    for (let i = 0; i < unref(selectRow).length; i++) {
      ...
      for (let j = 0; j < props.config.relationOptions.length; j++) {
        ...
        item[row.field] = row.type === 1 ? e[!unref(getIsDynamic) ? row.value + '_ceriId' : row.value] : row.value;
      }
      checked.push(item);
    }
    emit('select', checked);
  ...
  function getParamList() {
    ...
    for (let i = 0; i < templateJson.length; i++) {
      ...
      if (e.ceriKey === 'createUser') e.defaultValue = unref(getUserInfo).userId;
      if (e.ceriKey === 'createTime') e.defaultValue = new Date().getTime();
      if (e.ceriKey === 'currOrganize') e.defaultValue = unref(getUserInfo).organizeId;
      if (e.ceriKey === 'currPosition' && unref(getUserInfo).positionIds?.length) e.defaultValue = unref(getUserInfo).positionIds[0].id;
    }
    return templateJson;
  }
