    ...
    <a-table size="small" rowKey="id" class="complex-header-table" :data-source="list" :columns="columns" :pagination="false">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.key === 'field'">
          <CeriTreeSelect v-model:value="record.field" placeholder="请选择" showSearch allowClear :options="fieldOptions" class="!w-334px" />
        </template>
        <template v-if="column.key === 'type'">
          <CeriSelect v-model:value="record.type" placeholder="请选择" :options="typeList" class="!w-160px" />
