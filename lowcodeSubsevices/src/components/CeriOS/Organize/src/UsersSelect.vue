      ...
      <div class="transfer-pane left-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-else>
          ...
          <ScrollContainer v-loading="loading && pagination.currentPage === 1" ref="infiniteBody">
            ...
            <ceri-empty v-if="!ableList.length" />
          </ScrollContainer>
        </div>
      </div>
...
defineOptions({ name: 'CeriUsersSelect', inheritAttrs: false })
...
async function initData() {
  ...
  if (props.selectType === 'all') {
    ...
    loading.value = true
    if (activeKey.value === '2') ableList.value = []
    if (activeKey.value === '3') ableList.value = []
    if (activeKey.value === '4') ableList.value = []
