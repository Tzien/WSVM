<template>
  <div class="upload-file-container" :class="$attrs.class">
    <template v-if="!detailed">
      <a-button preIcon="icon-ym icon-ym-btn-upload" :disabled="disabled" @click="uploadFile">{{ buttonText
      }}</a-button>
      <p @click="handleDownloadAll" class="link-text" v-if="fileList.length && showDownload">
    ...
    <div class="upload-file-list" :class="{ 'upload-file-list__simple': simple }" v-if="showUploadList">
      <div class="upload-file-list__item" v-for="(file, index) in fileList" :key="file.fileId"
        :class="{ 'upload-file-list__item_detail': detailed }">
        <a class="upload-file-list__item-name"
        ...
        <span class="upload-file-list__item-actions">
          ...
          <CloseOutlined :title="t('component.upload.del')" @click="handleRemove(index)"
            v-show="!disabled && !detailed" />
        </span>
      </div>
    </div>
...
defineOptions({ name: 'CeriUploadFile', inheritAttrs: false });
