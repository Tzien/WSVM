...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'CeriRelationForm', inheritAttrs: false });
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      ...
      //   emitter.emit('setRelationData', { ceriRelationField: props.field, ...data, id: res.data.id });
      // });
    }
  }
  function setNullValue() {
    ...
    generatorStore.setRelationData(relationData);
    emitter.emit('setRelationData', { ceriRelationField: props.field });
  }
  function onChange() {
    options.value = [];
    emit('update:value', '');
