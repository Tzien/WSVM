...
export const getRealProps = (data, ceriKey) => {
...
export const getFormSchemaItem = data => {
  const t = getSafeT();
  // 兼容只配置了 __config__.ceriKey 的场景（如字典列表的 searchList）
  const ceriKey = data.__config__.ceriKey || '';
  const item = getRealProps(data, ceriKey);
