...
const routes = [
  ...
  },
  {
    ...
    name: '功能预览',
    i18nKey: 'routes.basic.previewModel',
    meta: {
...
router.beforeEach(async (to, from, next) => {
  ...
  const currentLanguage =
    (typeof navigationStore?.language === 'string' && navigationStore.language) ||
    (navigationStore?.language?.value ? navigationStore.language.value : '')
  // 检查是否需要重新加载路由
  const needReload =
    !routeStore.loaded ||
    lastUserRoles !== roles ||
    routeStore.routes.length === 0 ||
    `${routeStore.loadedLanguage || ''}` !== `${currentLanguage || ''}`
  if (needReload) {
    ...
    if (to.path === '/') {
      ...
      if (drawerStore) {
        if (typeof drawerStore.setFoldmenu === 'function') {
          drawerStore.setFoldmenu([`${import.meta.env.VITE_HOMEPAGEPATHFOLDMENU}`, innerPath])
        } else if (Array.isArray(drawerStore.foldmenu)) {
          drawerStore.foldmenu = [`${import.meta.env.VITE_HOMEPAGEPATHFOLDMENU}`, innerPath]
        }
