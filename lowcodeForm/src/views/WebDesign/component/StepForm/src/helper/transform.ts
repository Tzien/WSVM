...
import { useInputList, useDateList, useSelectList } from './config'
import { getSafeT } from '@/lang/safeI18n'

export function getRealProps(data, ceriKey) {
...
export const getFormSchemaItem = (data) => {
  const t = getSafeT()
  const item = getRealProps(data, data.__config__.ceriKey)
  ...
  const schema = {
    ...
    component: upperFirst(item.__config__.ceriKey),
    componentProps,
