      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-else>
          ...
          <template v-if="column.key === 'dataType'">
            <ceri-select v-model:value="record.dataType" placeholder="请选择" :options="options" />
          </template>
...
<script lang="ts" setup>
  ...
  import { getDataTypeText } from '@/utils/ceri';
  ...
  function exist() {
    ...
    for (let i = 0; i < fieldList.value.length; i++) {
      ...
      if (e.dataType == 'decimal' && !regex.test(e.dataLength)) {
  ...
  async function handleSubmit() {
    ...
    addTableFields(unref(linkId), query)
      .then(res => {
        createMessage.success(res.request.message);
