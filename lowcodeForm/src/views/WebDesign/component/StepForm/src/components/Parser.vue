<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, getParamList } from '@/utils/ceri';
  ...
  export default defineComponent({
    ...
    setup(props, { expose, emit }) {
      ...
      // 每个表单生成不同name保证id不重复
      const getFormName: string = `form-${guid()}`;
        ...
        colFormItem(element) {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.ceriKey === 'table' ? element.__vModel__ : undefined}
              relations={config.ceriKey === 'table' ? state.relations : undefined}
            />
          );
      ...
      expose({ handleSubmit, handleReset, getFormData });
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.ceriKey)) setTransferFormData(data, config);
      ...
      function setShowOrHide(prop, value) {
        ...
        if (!isChildTable) {
          ...
          updateFormConf(list[0], list[1], !newVal);
        }
      }
      function updateFormConf(table, prop, value) {
        const loop = list => {
          ...
          for (let i = 0; i < list.length; i++) {
            const data = list[i];
            if (list[i]?.__vModel__ && list[i]?.__vModel__ == table) {
              for (let j = 0; j < list[i].__config__.children.length; j++) {
                const item = list[i].__config__.children[j];
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.ceriKey) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.ceriKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.ceriKey == 'table') return;
      ...
      function initRelationForm(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (config.ceriKey == 'relationFormAttr' || config.ceriKey == 'popupAttr') {
            const relationKey = cur.relationField.split('_ceriTable_')[0];
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.ceriKey !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.ceriKey !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          ...
          if (config.ceriKey === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
      ...
      function getFormData() {
        const values = formElRef.value?.validate();
        if (!values) return;
