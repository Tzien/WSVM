<template>
  <a-form :colon="false" layout="vertical" class="right-board-form">
    <a-form-item label="多端显示">
      <ceri-checkbox v-model:value="activeData.__config__.visibility" :options="getVisibilityOptions" />
    </a-form-item>
    <template v-if="config.isSubTable && showType === 'pc'">
      <a-divider class="!mt-0">表格布局配置</a-divider>
      <a-form-item label="冻结方式">
        <ceri-radio v-model:value="activeData.__config__.tableFixed" :options="fixedOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
      <a-form-item label="对齐方式">
        <ceri-radio v-model:value="activeData.__config__.tableAlign" :options="alignOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
    ...
    <a-form-item label="标题宽度" v-if="!layoutList.includes(ceriKey)">
      <a-input-number v-model:value="activeData.__config__.labelWidth" placeholder="请输入" :min="0" />
    </a-form-item>
    <a-form-item v-if="!noWithList.includes(ceriKey) && activeData.style && showType === 'pc'">
      <template #label>
        控件宽度
    ...
    <a-form-item>
      ...
      <ceri-select v-model:value="activeData.__config__.className" multiple allowClear :options="getClassNameOptions" showSearch />
    </a-form-item>
  </a-form>
</template>
<script lang="ts" setup>
  ...
  const getVisibilityOptions = computed(() => (unref(ceriKey) == 'iframe' ? iframeVisibilityOptions : visibilityOptions));
</script>
