...
export function useField(activeData) {
  ...
  function initFieldData() {
    const ceriKey = activeData.__config__.ceriKey;
    const list = ['popupTableSelect', 'popupSelect', 'autoComplete'];
    if (list.includes(ceriKey)) {
    ...
    const url = list.includes(ceriKey) ? activeData.interfaceId : activeData.__config__.propsUrl;
