  ...
  setup(props, { attrs }) {
    ...
    if (!isQiankun) {
      ...
      watchEffect(() => {
        if (globalStore.value) {
          commonStore = globalStore.value.commonStore
        }
      })
    }
    let activeData = {}
    ...
    const components = {
      itemBtns(element, index, parent, isTableGrid = false) {
        const gutter = element.config_?.layout === 'colFormItem' && props.formConf?.gutter ? props.formConf.gutter : 0
    ...
    // 定义方法：根据传入的 dictId 获取低级项
    function getLowCodeDictItemsById(dictId) {
      // 扁平化数组并查找对应的 dictId
      const targetItems = commonStore.dictOptions.flatMap((category) => category.lowCodeDictInfos).find((dictInfo) => dictInfo.dictId === dictId)

      // 如果找到了该项，返回子项，否则返回空数组
      return targetItems ? targetItems.lowCodeDictItems : []
    }
    const layouts = {
      colFormItem(element, index, parent) {
        const { onActiveItem } = attrs as unknown as AttrsType
        const config = element.__config__
        const className = props.activeId === config.formId ? 'drawing-item active-from-item' : 'drawing-item aaaa'
        ...
        if (config.ceriKey == 'location') element.useAutoLocation = false
        const Item =
          props.showType === 'app' && !useCompList.includes(config.ceriKey) ? (
        ...
        return (
          <a-col
            ...
            draggable={false}
            onClick={(event) => {
              if (props.element.__config__.dictionaryType) {
                getLowCodeDictItemsById(props.element.__config__.dictionaryType)
                props.element.options = getLowCodeDictItemsById(props.element.__config__.dictionaryType).map((item) => ({
                  fullName: item.dictItemName,
      ...
      rowFormItem(element, index, parent) {
        ...
        if (element.__config__.ceriKey === 'tableGrid') {
        ...
        if (config.ceriKey === 'row' || (props.showType === 'app' && config.ceriKey === 'card')) {
          return (
            <a-col
              span={props.showType === 'app' ? 24 : config.span}
              data-draggable={true}
            ...
            >
              <a-row class={className + ' drawing-row-item-row'}>
                <span class="component-name">{config.ceriKey === 'card' ? config.label : config.componentName}</span>
                <a-col>
