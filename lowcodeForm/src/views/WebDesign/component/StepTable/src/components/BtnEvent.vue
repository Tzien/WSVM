    ...
    <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="formRules" ref="formElRef" hideRequiredMark>
      ...
      <template v-if="dataForm.btnType == 1">
        <a-form-item label="选择表单" name="modelId">
          <ceri-tree-select v-model:value="dataForm.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-form-item label="弹窗类型" name="popupType" v-if="showType === 'pc'">
          <ceri-select v-model:value="dataForm.popupType" placeholder="请选择" :options="popupTypeOptions" />
        </a-form-item>
        ...
        <a-table :data-source="dataForm.formOptions" :columns="formOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'field'">
              <ceri-select v-model:value="record.field" placeholder="请选择" :options="fieldOptions" showSearch @dropdownVisibleChange="visibleChange" />
            </template>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'relationField'">
                ...
                <template v-else-if="record.sourceType == 2">
                  <ceri-input-number v-if="['int', 'decimal'].includes(record.dataType)" v-model:value="record.relationField" placeholder="请输入" allowClear />
                  <ceri-date-picker
...
<script lang="ts" setup>
  ...
  import ScriptDemo from '@/views/WebDesign/component/StepForm/src/components/ScriptDemo.vue';
  import { noAllowSelectList } from '@/views/WebDesign/component/StepForm/src/helper/config';
  ...
  import LaunchFlowCfg from '@/views/WebDesign/component/StepForm/src/components/LaunchFlowCfg.vue';
  ...
  function init(data) {
    ...
    formFieldsOptions.value = cloneDeep(data.formFieldsOptions)
      .filter(o => !noAllowFormFieldsList.includes(o.__config__.ceriKey) && o.id.indexOf('-') < 0)
