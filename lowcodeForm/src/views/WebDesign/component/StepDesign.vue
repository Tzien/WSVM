      ...
      <template #title>
        <div class="StepDesignContainerTop">
          <div class="StepDesignContainerStep">
            <a-steps :current="activeStep" type="navigation" size="small" class="header-steps" @change="handleStepChange">
              <a-step title="基础设计" />
...
const next = async () => {
  if (state.activeStep == 0) {
    ...
    const currentFormDesignId = state.dataForm.formDesignId
    state.dataForm = { ...state.dataForm, ...formvalidate.data }
    if (currentFormDesignId) state.dataForm.formDesignId = currentFormDesignId
    getDbType()
...
async function handleStepChange(targetStep) {
  ...
  if (targetStep < state.activeStep) {
    while (state.activeStep > targetStep) {
      prev()
    }
    return
  }
  while (state.activeStep < targetStep) {
    ...
    await next()
    if (state.activeStep === currentStep) return
  }
}
const prev = () => {
...
function handleRequest() {
  ...
  if (Array.isArray(state.dataForm.formDbs)) {
    state.dataForm.formDbs = state.dataForm.formDbs.map((item) => {
      if (!!!item.formDbId) item.formDbId = ''
      const fields = item && Object.prototype.hasOwnProperty.call(item, 'fields') ? item.fields : undefined
      return {
        ...item,
        fields: typeof fields === 'string' ? fields : fields != null ? JSON.stringify(fields) : null
      }
    })
  }
  ...
  formMethod(query)
    .then((res) => {
      ...
      if (!state.dataForm.formDesignId && createdId) state.dataForm.formDesignId = createdId
    })
    .catch((e) => {
