...
function parseTableFields(fields) {
  ...
  if (data.formDbs?.length > 0) {
    ...
    for (let i = 0; i < data.formDbs.length; i++) {
      ...
      const item = {
        ...
        tableField: '',
        typeId: e.typeId,
        fields: parseTableFields(e.fields)
      }
...
async function addNewConnectData() {
  ...
  const modalPayload = { dbId: formState.dbId }
  if (formState.formDesignId || ConnectDataSource.value.length) {
    modalPayload.selectedTables = ConnectDataSource.value.map((o) => o.tableName || o.table)
  }
  openTableModal(true, modalPayload)
}
