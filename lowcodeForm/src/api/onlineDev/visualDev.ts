...
// 获取视图列表
export function getViewList(menuId, userId) {
  return lowCodeFormInstance.get(`/api/FormDb/GetViewList?menuId=${menuId}&userId=${userId}`)
}
...
export function setDefaultView(id, menuId) {
  return lowCodeFormInstance.put(Api.PersonalPrefix + `/${id}/setDefault?menuId=${menuId}`)
}
...
export function codePreview(id, data) {
  return lowCodeFormInstance.post(`/api/FormDb/CodePreview/${id}`, data)
}
