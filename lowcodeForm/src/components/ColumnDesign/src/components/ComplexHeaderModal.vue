<template>
  <BasicModal v-bind="$attrs" width="1000px" class="CERI-complex-header-Modal" @register="registerModal" title="复杂表头配置" @ok="handleSubmit" destroyOnClose>
    <a-table size="small" rowKey="id" class="complex-header-table" :data-source="list" :columns="columns" :pagination="false">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.key === 'fullName'">
          <ceri-i18n-input v-model:value="record.fullName" v-model:i18n="record.fullNameI18nCode" placeholder="请输入" allowClear :maxlength="50" />
        </template>
        ...
        <template v-if="column.key === 'align'">
          <CeriSelect v-model:value="record.align" placeholder="请选择" :options="alignList" />
        </template>
