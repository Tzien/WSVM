...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      const ceriKey = confClone.__config__.ceriKey;
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(ceriKey)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(ceriKey)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(ceriKey)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_ceriTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const ceriKey = upperFirst(props.conf.__config__.ceriKey) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.ceriKey);
      const Comp = componentMap.get(ceriKey as ComponentType) as ReturnType<typeof defineComponent>;
      if (!Comp) return null;
      const realDataObject = getRealProps(dataObject, props.conf.__config__.ceriKey);
