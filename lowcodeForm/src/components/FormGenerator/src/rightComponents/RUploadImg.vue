<template>
  <a-form-item label="上传类型">
    <ceri-radio v-model:value="activeData.showType" :options="showTypeOptions" optionType="button" button-style="solid" />
  </a-form-item>
  ...
  <a-form-item label="上传路径">
    <ceri-radio v-model:value="activeData.pathType" :options="pathTypeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <template v-if="activeData.pathType === 'selfPath'">
    <a-form-item label="排序规则">
      <ceri-select v-model:value="activeData.sortRule" :options="sortRuleOptions" multiple />
    </a-form-item>
    <a-form-item label="时间格式" v-if="activeData.sortRule?.includes('2')">
      <ceri-select v-model:value="activeData.timeFormat" :options="timeFormatOptions" />
    </a-form-item>
