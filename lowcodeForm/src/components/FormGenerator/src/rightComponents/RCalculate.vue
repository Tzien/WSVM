<template>
  ...
  <a-form-item label="舍入类型">
    <ceri-select v-model:value="activeData.roundType" :options="roundTypeList" />
  </a-form-item>
...
<script lang="ts" setup>
  ...
  const calculateComps = computed<any[]>(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__config__ && data.__config__.ceriKey && data.__vModel__) {
        if (['inputNumber', 'calculate'].includes(data.__config__.ceriKey)) {
