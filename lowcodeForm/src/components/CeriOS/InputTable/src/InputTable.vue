<template>
  <div :class="prefixCls">
    <CeriGroupTitle
      ...
      :bordered="false" />
    <div class="ceri-child-list" v-if="config.layoutType === 'list'">
      <a-collapse expandIconPosition="end" :bordered="false" class="outer-collapse" v-model:activeKey="state.outerActiveKey">
        <a-collapse-panel forceRender>
          ...
          <a-collapse :bordered="false" v-model:activeKey="state.innerActiveKey">
            ...
            <a-collapse-panel v-for="(record, index) in dataSource" :key="record.ceriId" forceRender>
              <template #header>
              ...
              <a-row :gutter="formConf.formStyle ? 0 : formConf.gutter">
                <a-col :span="column.__config__.span" v-for="(column, cIndex) in rowConfig" :key="column.__config__.formId">
                  <a-form-item
                    ...
                    <CeriRelationForm
                      v-if="column.__config__.tag === 'CeriRelationForm'"
                      :rowIndex="index"
                ...
                <template v-for="(column, cIndex) in rowConfig" :key="column.__config__.formId">
                  <a-col :span="column.__config__.span" v-if="column.__vModel__ && config.summaryField.includes(column.__vModel__)">
                    <a-form-item :labelCol="column.labelCol">
                      ...
                      <CeriInput :value="getColumnSum[cIndex]" disabled :style="column.style" />
                    </a-form-item>
                  </a-col>
                </template>
              </a-row>
        ...
        <template v-for="(item, cIndex) in rowConfig">
          <template v-if="column.key === item.__vModel__ && column.__config__.formId === item.__config__.formId">
            <div :key="item.__config__.formId">
              <CeriRelationForm
                v-if="item.__config__.tag === 'CeriRelationForm'"
                :rowIndex="index"
                :tableVModel="config.__vModel__"
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, getDateTimeUnit, thousandsFormat, onlineUtils } from '@/utils/ceri';
  ...
  defineOptions({ name: 'CeriInputTable', inheritAttrs: false });
  ...
  const getSystemField = computed(() => {
    let systemList: any[] = [];
    state.rowConfig.map(o => systemComponentsList.includes(o.__config__.ceriKey) && systemList.push(o.__vModel__));
    return systemList;
  });
  ...
  function buildOptions() {
    state.rowConfig.forEach((cur, index) => {
      ...
      if (dyOptionsList.indexOf(config.ceriKey) > -1) {
    ...
    for (let key in currRelations) {
      if (key === data.__vModel__) {
        for (let i = 0; i < currRelations[key].length; i++) {
          ...
          const ceriKey = config.ceriKey;
          let defaultValue: any = null;
          if (
            ['checkbox', 'cascader', 'organizeSelect'].includes(ceriKey) ||
            (['select', 'treeSelect', 'popupSelect', 'popupTableSelect', 'userSelect'].includes(ceriKey) && e.multiple)
          ) {
            defaultValue = [];
          }
  ...
  function buildRowAttr(rowIndex) {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (config.ceriKey === 'userSelect' && cur.relationField && cur.selectType !== 'all' && cur.selectType !== 'custom') {
        let value = getFieldVal(cur.relationField, rowIndex) || [];
        cur.ableRelationIds = Array.isArray(value) ? value : [value];
      }
      if (config.ceriKey === 'datePicker' || config.ceriKey === 'timePicker') {
  ...
  function onFormDataChange(rowIndex, colIndex, val, row) {
    ...
    if (['popupSelect', 'relationForm'].includes(data.ceriKey)) setTransferFormData(row, data.config.__config__);
  }
  ...
  function getConfById(formId, rowIndex) {
    ...
    let newObj: any = {};
    item = getRealProps(item, itemConfig.ceriKey);
    ...
    if (['relationForm', 'popupSelect'].includes(itemConfig.ceriKey)) {
      newObj['field'] = props.config.__vModel__ + item.__vModel__ + '_ceriRelation_' + state.dataSource[rowIndex].ceriId;
      delete newObj.extraOptions;
    }
    if (['relationFormAttr', 'popupAttr'].includes(itemConfig.ceriKey)) {
      let prop = newObj['relationField'].split('_ceriTable_')[0];
      newObj['relationField'] = props.config.__vModel__ + prop + '_ceriRelation_' + state.dataSource[rowIndex].ceriId;
    }
    return newObj;
  }
  ...
  function getEmptyRow(val) {
    return state.rowConfig.map((t: any) => ({
      ...
      options: dyOptionsList.indexOf(t.__config__.ceriKey) > -1 ? t.options : [],
      valid: true,
      regValid: true,
      regErrorText: '',
      on: t.on || {},
      ceriKey: t.__config__.ceriKey,
  ...
  function getDefaultEmptyValue(item) {
    if (!item.__config__ || !item.__config__.ceriKey) return undefined;
    const ceriKey = item.__config__.ceriKey;
    const list = ['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'];
    return list.includes(ceriKey) || item.multiple ? [] : undefined;
  }
  // 获取表格数据
  ...
  function addRow(val?, isUpdate = true) {
    ...
    const ceriId = buildUUID();
    rowData.ceriId = ceriId;
    ...
    nextTick(() => {
      if (isUpdate) {
        if (props.config.layoutType === 'list' && props.config.defaultExpandAll) state.innerActiveKey.push(ceriId);
        updateParentData();
      }
      clearAddRowFlag();
    });
  ...
  function batchRemoveRow(item) {
    ...
    const handleBatchRemove = () => {
      state.tableFormData = state.tableFormData.filter(o => !state.selectedRowKeys.includes(o.ceriId));
      state.dataSource = state.dataSource.filter(o => !state.selectedRowKeys.includes(o.ceriId));
      nextTick(() => {
  ...
  function setActiveKey() {
    ...
    for (let i = 0; i < state.dataSource.length; i++) {
      state.innerActiveKey.push(state.dataSource[i].ceriId);
    }
  }
  // 初始化
  function init() {
