<template>
  ...
  <a-modal v-model:open="visible" title="省市区" :width="800" class="transfer-modal" @ok="handleSubmit" centered :maskClosable="false" :keyboard="false">
    <template #closeIcon>
    ...
    <div class="transfer__body">
      <div class="transfer-pane">
        ...
        <div class="transfer-pane__body">
          ...
          <a-tree :tree-data="treeData" selectable @select="handleSelect" />
        </div>
      </div>
...
import pkg from 'province-city-china/data'
const { province, city, area } = pkg
...
defineOptions({ name: 'ceriAreaSelect', inheritAttrs: false })
...
//   console.log('点击节点:', info.node)
//   if (info.node.children) {
//     console.log('下级节点:', info.node.children)
//   } else {
//     console.log('没有下级节点')
//   }
// }
function buildTreeData() {
  return province.map((p) => {
    // 所有属于这个省的城市
    let cityList = city
      .filter((c) => c.province == p.province)
      .map((c) => {
        // 所有属于这个市的区县
        const areaList = area
          .filter((a) => a.city === c.city && a.province == c.province)
          .map((a) => ({
            title: a.name,
            value: a.code,
            key: a.code
    ...
    // 如果 cityList 为空（直辖市），直接把区放到省下面
    if (cityList.length === 0) {
      cityList = area
        .filter((a) => a.province == p.province)
        .map((a) => ({
          title: a.name,
          value: a.code,
          key: a.code
...
function handleSelect(_keys, { node }) {
  ...
  if (props.multiple) {
      // 多选，避免重复
      const exists = selectedData.value.some(d => d.id === item.id)
      if (!exists) {
