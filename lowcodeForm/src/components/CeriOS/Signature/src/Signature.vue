...
<script lang="ts" setup>
  ...
  import { ref, unref, watch, computed,watchEffect } from 'vue';
  ...
  defineOptions({ name: 'CeriSignature', inheritAttrs: false });
  ...
  function openSignModal() {
    ...
    const data = {name:name.value, value: innerValue.value, ableIds: props.ableIds };
    signRef?.openModal(data);
  }
  const name=ref('')
  function handleSubmit(value) {
    name.value=value.name
    innerValue.value = value.src;
