<template>
  <div :class="[prefixCls, $attrs.class]">
    <a-input v-model:value="innerValue" :size="$attrs.size || 'default'" :placeholder="placeholder" readonly allowClear>
      <template #addonAfter>
        <span @click="openSelectModal">{{ t('component.ceri.iconPicker.select') }}</span>
      </template>
    ...
    <a-modal v-model:open="visible" :width="1000" class="icon-modal" centered :maskClosable="false" :keyboard="false">
      <template #title>
        <div class="icon-modal-title">
          {{ t('component.ceri.iconPicker.modalTitle') }}
          <a-input-search :placeholder="t('component.ceri.iconPicker.searchPlaceholder')" allowClear v-model:value="keyword" @search="handleSearch" />
        </div>
      </template>
      <template #closeIcon>
      ...
      <div class="main">
        <a-tabs v-model:activeKey="activeKey">
          <a-tab-pane key="1" :tab="t('component.ceri.iconPicker.ymIcon')">
            <ScrollContainer>
          ...
          <a-tab-pane key="2" :tab="t('component.ceri.iconPicker.ymCustom')">
            <ScrollContainer>
              <div class="icon-box-list">
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'CeriIconPicker', inheritAttrs: false });
