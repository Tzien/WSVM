...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'CeriRelationFormAttr', inheritAttrs: false });
  ...
  const getBindValue = computed(() => ({
    ...
    placeholder: props.isStorage ? t('用于展示关联表单的属性，且数据同时会保存入库') : t('用于展示关联表单的属性，且数据不会保存'),
  }));
  ...
  //     if (!props.isStorage || !props.showField || !props.relationField || props.relationField != data.ceriRelationField) return;
