<template>
  <div :class="[$attrs.class, 'select-tag-list']" v-if="buttonType === 'button'">
    <a-button preIcon="icon-ym icon-ym-btn-add" @click="openSelectModal">123{{ modalTitle }}</a-button>
    <div class="tags">
...
async function getList() {
  ...
  const data = await useGetOraganizeTreeAsync()
...
function convertTreeData(data) {
  return data.map((item) => ({
    ...
    fullName:item.name,
    children:
      item.oraganizeTrees && item.oraganizeTrees.length
        ? convertTreeData(item.oraganizeTrees)
        : [],
  }))
}
