    ...
    <div class="transfer__body">
      <div class="transfer-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab">
          <a-tree :tree-data="filteredTreeData" default-expand-all @select="handleNodeSelect" />
      ...
      <div class="transfer-pane right-pane">
        <div class="transfer-pane__tool">
          ...
          <span class="remove-all-btn" @click="removeAll">{{ t('component.ceri.common.clearAll') }}</span>
        </div>
...
import { getRoleInfo } from '@/api/system/organization'

defineOptions({ name: 'CeriPosSelect', inheritAttrs: false })
...
function handleSubmit() {
  ...
  handleCancel()
  console.info(333,options.value,innerValue.value)
}
...
async function getList() {
  ...
  if (data.code === 200 && data.success) {
    treeData.value = convertTreeData(data.data)
  }
}
function convertTreeData(data) {
  return data.map((item) => ({
    id:item.id,
    key: item.id,          // 对应 a-tree 默认 key
    title: item.name,      // 对应 a-tree 默认 title
    fullName:item.name,
    children:
      item.children && item.children.length
        ? convertTreeData(item.children)
        : [],
  }))
}
</script>
