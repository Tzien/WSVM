...
/// 业务实现：演示示例Demo.
/// </summary>
[ApiDescriptionSettings(Tag = "示例", Name = "User", Order = 200)]
[Route("api/[controller]")]
public class UserService : ControllerBase, IUserService
{
    public Repository<UserEntity> _db { get; set; }

    /// <summary>
    /// 初始化一个<see cref="UserService"/>类型的新实例.
    /// </summary>
    public UserService(Repository<UserEntity> db)
    {
       _db = db;
    }
        ...
        private static IDictionary<string, JsonElement> GetExtraFilters(object input)
   {
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<UserInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] UserListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(UserEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
        var query = _db.Context.Queryable<UserEntity>();
        ...
        var inlineEditorList = data.list.Adapt<List<UserListOutput>>(); 
        return new
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] UserCrInput input)
    {
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] UserUpInput input)
    {
        var entity = input.Adapt<UserEntity>();
    ...
    {
        var isOk = await _db.Context.Deleteable<UserEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();   
        if (isOk == 0)
        {
            return new
            {
    ...
    public async Task<dynamic> BatchRemove([FromBody] List<string>? delids)
    {
        ...
        if (ids.Count > 0)
        {
            var entitys = await _db.Context.Queryable<UserEntity>().In(it => it.id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                 // 批量删除演示示例Demo
                await _db.Context.Deleteable<UserEntity>().In(it => it.id, ids).ExecuteCommandAsync();
            }
        }
        return new
        {
