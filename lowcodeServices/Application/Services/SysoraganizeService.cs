...
/// <summary>
/// 业务实现：AddNewTableTest.
/// </summary>
[ApiDescriptionSettings(Tag = "示例", Name = "Sysoraganize", Order = 200)]
[Route("api/[controller]")]
public class SysoraganizeService : ControllerBase, ISysoraganizeService
{
    public Repository<SysoraganizeEntity> _db { get; set; }

    /// <summary>
    /// 初始化一个<see cref="SysoraganizeService"/>类型的新实例.
    /// </summary>
    public SysoraganizeService(Repository<SysoraganizeEntity> db)
    {
       _db = db;
    }
        ...
        private static IDictionary<string, JsonElement> GetExtraFilters(object input)
   {
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<SysoraganizeInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] SysoraganizeListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(SysoraganizeEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
      var query = _db.Context.Queryable<SysoraganizeEntity>();
       ...
       var data = await query.ToPagedListAsync(input.currentPage, input.pageSize);
        var inlineEditorList = data.list.Adapt<List<SysoraganizeListOutput>>();
        return PageResult<SysoraganizeListOutput>.SqlSugarPageResult(new SqlSugarPagedList<SysoraganizeListOutput>
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] SysoraganizeCrInput input)
    {
        var entity = input.Adapt<SysoraganizeEntity>();
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] SysoraganizeUpInput input)
    {
        var entity = input.Adapt<SysoraganizeEntity>();
    ...
    {
        var isOk = await _db.Context.Deleteable<SysoraganizeEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();   
        if (isOk == 0)
        {
            return new
            {
    ...
    public async Task<dynamic> BatchRemove([FromBody] List<string>? delids)
    {
        ...
        if (ids.Count > 0)
        {
            var entitys = await _db.Context.Queryable<SysoraganizeEntity>().In(it => it.id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                 // 批量删除AddNewTableTest
                await _db.Context.Deleteable<SysoraganizeEntity>().In(it => it.id, ids).ExecuteCommandAsync();
            }
        }
        return new
        {
