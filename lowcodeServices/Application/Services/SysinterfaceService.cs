...
/// 业务实现：测试等于查询2.
/// </summary>
[ApiDescriptionSettings(Tag = "示例", Name = "Sysinterface", Order = 200)]
[Route("api/[controller]")]
public class SysinterfaceService : ControllerBase, ISysinterfaceService
{
    public Repository<SysinterfaceEntity> _db { get; set; }

    /// <summary>
    /// 初始化一个<see cref="SysinterfaceService"/>类型的新实例.
    /// </summary>
    public SysinterfaceService(Repository<SysinterfaceEntity> db)
    {
        _db = db;
    }
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<SysinterfaceInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] SysinterfaceListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(SysinterfaceEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
        var query = _db.Context.Queryable<SysinterfaceEntity>();
        ...
        var inlineEditorList = data.list.Adapt<List<SysinterfaceListOutput>>();
        return new
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] SysinterfaceCrInput input)
    {
        var entity = input.Adapt<SysinterfaceEntity>();
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] SysinterfaceUpInput input)
    {
        var entity = input.Adapt<SysinterfaceEntity>();
    ...
    {
        var isOk = await _db.Context.Deleteable<SysinterfaceEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();
        if (isOk == 0)
        {
            return new
            {
    ...
    public async Task<dynamic> BatchRemove([FromBody] List<string>? delids)
    {
        ...
        if (ids.Count > 0)
        {
            var entitys = await _db.Context.Queryable<SysinterfaceEntity>().In(it => it.id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                // 批量删除测试等于查询2
                await _db.Context.Deleteable<SysinterfaceEntity>().In(it => it.id, ids).ExecuteCommandAsync();
            }
        }
        return new
        {
