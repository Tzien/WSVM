...
/// 业务实现：测试表单设计绑定列关系.
/// </summary>
[ApiDescriptionSettings(Tag = "示例", Name = "Userinfo", Order = 200)]
[Route("api/[controller]")]
public class UserinfoService : ControllerBase, IUserinfoService
{
    public Repository<UserinfoEntity> _db { get; set; }

    /// <summary>
    /// 初始化一个<see cref="UserinfoService"/>类型的新实例.
    /// </summary>
    public UserinfoService(Repository<UserinfoEntity> db)
    {
       _db = db;
    }
        ...
        private static IDictionary<string, JsonElement> GetExtraFilters(object input)
   {
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<UserinfoInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] UserinfoListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(UserinfoEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
      var query = _db.Context.Queryable<UserinfoEntity>();
       ...
       var data = await query.ToPagedListAsync(input.currentPage, input.pageSize);
        var inlineEditorList = data.list.Adapt<List<UserinfoListOutput>>();
        return PageResult<UserinfoListOutput>.SqlSugarPageResult(new SqlSugarPagedList<UserinfoListOutput>
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] UserinfoCrInput input)
    {
        var entity = input.Adapt<UserinfoEntity>();
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] UserinfoUpInput input)
    {
        var entity = input.Adapt<UserinfoEntity>();
    ...
    {
        var isOk = await _db.Context.Deleteable<UserinfoEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();   
        if (isOk == 0)
        {
            return new
            {
    ...
    public async Task<dynamic> BatchRemove([FromBody] List<string>? delids)
    {
        ...
        if (ids.Count > 0)
        {
            var entitys = await _db.Context.Queryable<UserinfoEntity>().In(it => it.id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                 // 批量删除测试表单设计绑定列关系
                await _db.Context.Deleteable<UserinfoEntity>().In(it => it.id, ids).ExecuteCommandAsync();
            }
        }
        return new
        {
