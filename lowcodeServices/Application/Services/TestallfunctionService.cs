...
public class TestallfunctionService : ControllerBase, ITestallfunctionService
{
    public Repository<TestallfunctionEntity> _db { get; set; }

    static TestallfunctionService()
    {
        var cfg = TypeAdapterConfig.GlobalSettings;
        new CeriOS.示例.Entitys.Mapper.Testallfunction.Mapper().Register(cfg);
    }

    /// <summary>
    /// 初始化一个<see cref="TestallfunctionService"/>类型的新实例.
    /// </summary>
    public TestallfunctionService(Repository<TestallfunctionEntity> db)
    {
       _db = db;
    }
   ...
   private static string JsonToString(object input)
   {
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<TestallfunctionInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] TestallfunctionListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(TestallfunctionEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
      var query = _db.Context.Queryable<TestallfunctionEntity>();
      ...
      var inlineEditorList = data.list.Adapt<List<TestallfunctionListOutput>>();
        return new
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] TestallfunctionCrInput input)
    {
        if (input == null)
        {
            return new
        ...
        var entity = input.Adapt<TestallfunctionEntity>();
        entity.FileURL = input.FileURL != null && input.FileURL.Count > 0 ? JsonToString(input.FileURL).Replace("\r\n", "").Replace(" ", "") : null;
        entity.PictureURL = input.PictureURL != null && input.PictureURL.Count > 0 ? JsonToString(input.PictureURL).Replace("\r\n", "").Replace(" ", "") : null;
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] TestallfunctionUpInput input)
    {
        var entity = input.Adapt<TestallfunctionEntity>();
        entity.FileURL = input.FileURL != null && input.FileURL.Count > 0 ? JsonToString(input.FileURL).Replace("\r\n", "").Replace(" ", "") : null;
        entity.PictureURL = input.PictureURL != null && input.PictureURL.Count > 0 ? JsonToString(input.PictureURL).Replace("\r\n", "").Replace(" ", "") : null;
    ...
    {
        var isOk = await _db.Context.Deleteable<TestallfunctionEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();   
        if (isOk == 0)
        {
            return new
            {
    ...
    {
          var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<TestallfunctionDetailOutput>();
          return new
          {
             code = 200,
             msg = "获取成功",
