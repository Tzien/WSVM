...
/// 业务实现：测试等于查询.
/// </summary>
[ApiDescriptionSettings(Tag = "示例", Name = "Mediainfo", Order = 200)]
[Route("api/[controller]")]
public class MediainfoService : ControllerBase, IMediainfoService
{
    public Repository<MediainfoEntity> _db { get; set; }

    /// <summary>
    /// 初始化一个<see cref="MediainfoService"/>类型的新实例.
    /// </summary>
    public MediainfoService(Repository<MediainfoEntity> db)
    {
       _db = db;
    }
        ...
        private static IDictionary<string, JsonElement> GetExtraFilters(object input)
   {
    ...
    {
        var data = (await _db.GetFirstAsync(it => it.id.Equals(id))).Adapt<MediainfoInfoOutput>();
        return new
        {
            code = 200,
            msg = "获取成功",
    ...
    /// <returns></returns>
    [HttpPost("List")]
    public async Task<dynamic> GetList([FromBody] MediainfoListQueryInput input)
    {
        var entityInfo = _db.Context.EntityMaintenance.GetEntityInfo(typeof(MediainfoEntity));
        var selectIds = input.selectIds?.Split(",").ToList();
      var query = _db.Context.Queryable<MediainfoEntity>();
       ...
       var data = await query.ToPagedListAsync(input.currentPage, input.pageSize);
        var inlineEditorList = data.list.Adapt<List<MediainfoListOutput>>();
        return PageResult<MediainfoListOutput>.SqlSugarPageResult(new SqlSugarPagedList<MediainfoListOutput>
        {
    ...
    /// <returns></returns>
    [HttpPost("")]
    public async Task<dynamic> Create([FromBody] MediainfoCrInput input)
    {
        var entity = input.Adapt<MediainfoEntity>();
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task<dynamic> Update(string id, [FromBody] MediainfoUpInput input)
    {
        var entity = input.Adapt<MediainfoEntity>();
    ...
    {
        var isOk = await _db.Context.Deleteable<MediainfoEntity>().Where(it => it.id.Equals(id)).ExecuteCommandAsync();   
        if (isOk == 0)
        {
            return new
            {
    ...
    public async Task<dynamic> BatchRemove([FromBody] List<string>? delids)
    {
        ...
        if (ids.Count > 0)
        {
            var entitys = await _db.Context.Queryable<MediainfoEntity>().In(it => it.id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                 // 批量删除测试等于查询
                await _db.Context.Deleteable<MediainfoEntity>().In(it => it.id, ids).ExecuteCommandAsync();
            }
        }
        return new
        {
