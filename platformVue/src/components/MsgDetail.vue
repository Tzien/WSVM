<template>
    ...
    <!-- 消息详情弹框 -->
    <a-modal v-model:open="open" title="消息详情" centered :keyboard="false"  width="30%" :footer="null" @cancel="cancel">
        <a-form ref="formRef" :model="formState" :label-col="labelCol" style="height: 700px;">
            <a-form-item label="消息类型">
                <a-input disabled v-model:value="formState.msgType" style="width: 300px" />
            </a-form-item>
            <a-form-item label="标题" name="code">
                <a-input disabled v-model:value="formState.title" style="width: 300px" />
            </a-form-item>
            <a-form-item label="内容">
                <a-textarea disabled v-model:value="formState.content" style="width: 300px" 
        ...
        <div v-show="formState.isRead == '未读'" style="text-align: center;">
            <a-button @click="readMsg">收到</a-button>
        </div>
    </a-modal>
</template>
...
const emit = defineEmits(['read-success'])

import { useSignalRStore } from '@/store/index.js'
import { ref, reactive, onMounted, watch, onBeforeUnmount,watchEffect } from 'vue'
...
async function detail() {
    ...
    if (response.code === 200 && response.success) {
        ...
        if(response.data.msgType=='1')
            formState.msgType = '公告信息通知'
        else if(response.data.msgType=='2')
            formState.msgType = '微信消息通知'
        else if(response.data.msgType=='3')
            formState.msgType = '钉钉消息通知'
        formState.sendTime = response.data.sendTime.replace('T', ' ')
        formState.sendUser = response.data.sendUser
        formState.isRead = response.data.isRead ? '已读' : '未读'
        if (response.data.msgReadTime !== null)
            formState.msgReadTime = response.data.msgReadTime.replace('T', ' ')
    }
...
const readMsg = async () => {
  console.info('子应用-关闭')
//   signalRStore.closeCurrentNotification()
