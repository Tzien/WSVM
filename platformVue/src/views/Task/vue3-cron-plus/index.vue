  ...
  <div class="vue3-cron-plus-container">
    ...
    <el-tabs type="border-card">
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Seconds.name }}</span
          >
        </template>
      ...
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Minutes.name }}</span
          >
        </template>
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          <el-row>
      ...
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Hours.name }}</span
          >
        </template>
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          <el-row>
      ...
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Day.name }}</span
          >
        </template>
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          <el-row>
      ...
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Month.name }}</span
          >
        </template>
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          <el-row>
      ...
      <el-tab-pane>
        <template #label>
          <span
            ><el-icon><Calendar /></el-icon> {{ state.text.Year.name }}</span
          >
        </template>
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          <el-row>
    ...
    <div class="bottom">
      <!-- <div class="value"> -->
      <!-- <span> cron预览: </span> -->
      <a-input v-model:value="state.cron" addon-before="Cron表达式：" />
      <h3 style="margin-top: 10px">最近5次</h3>
      <a-list size="small" bordered :data-source="state.cronData">
...
<script>
import { getTaskeFireTimeApi } from '@/api/Task/task'
import Language from './language'
import { reactive, computed, toRefs, defineComponent, watch, onMounted } from 'vue'
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const state = reactive({
      ...
      cron: computed(() => {
        return `${state.secondsText || '*'} ${state.minutesText || '*'} ${state.hoursText || '*'} ${state.daysText || '*'} ${state.monthsText || '*'} ${
          state.weeksText || '?'
        } ${state.yearsText || '*'}`
      })
    })
    ...
    onMounted(() => {
      getTaskeFireTime(state.cron)
    })
    watch(
      () => state.cron,
      (newValue, oldValue) => {
        getTaskeFireTime(newValue)
      }
    )
    function getTaskeFireTime(cron) {
      const query = {
        CronExpressionString: cron
      }
      getTaskeFireTimeApi(query).then((res) => {
        if (res.code === 200 && res.success) {
          state.cronData = res.data
        }
      })
    }
    return {
