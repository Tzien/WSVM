<template>
  <div class="MenuAccessStyle">
    <div style="height: 35px; line-height: 35px; text-align: right; margin-right: 10px">
      <!-- <span>
        系统名称：
      ...
      <span>
        IP地址：
        <a-input v-model:value="ipKey" placeholder="请输入IP地址" style="width: 150px" />
      </span>
      <span style="margin-left: 10px">
        菜单名称：
        <a-input v-model:value="menuName" placeholder="请输入菜单名称" style="width: 150px" />
      </span>
      <span style="margin-left: 10px">
        访问时间：
        <a-date-picker
        ...
        />
        <a-date-picker show-time v-model:value="endTime" style="width: 150px" valueFormat="YYYY-MM-DD HH:mm:ss" placeholder="终止于" @change="onChangeEnd" />
      </span>
      <span style="margin-left: 10px">
        访问人：
        <a-select
      ...
      <span style="margin-left: 10px">
        角色名称：
        <a-tree-select
          v-model:value="sysRoleSelectValue"
          show-search
          style="width: 150px; text-align: left"
    ...
    <a-table
      ...
      :data-source="data"
      :pagination="false"
      :scroll="{ y: 'calc(100vh - 96px - 70px - 55px - 35px - 40px)' }"
    >
      <template #bodyCell="{ text, column, record }">
        <template v-if="column.dataIndex === 'createTime'">
          {{ formatDateTime(text) }}
        </template>
        <template v-if="record.menuId === 'zhsy'">
...
import { getSysMenuRecord, deleteAllApi, deleteSelectApi } from '@/api/MenuSysLog'
import { getSysinfoList, getUserInfo, getRoleInfo } from '@/api/commonFun'
...
const inputFunctionName = ref('A0301')
...
const sysRoleSelectValue = ref(null)
const sysRoleOptions = ref([])
...
async function getRoleList() {
  await getRoleInfo().then((res) => {
    if (res.code == 200 && res.success) {
      sysRoleOptions.value = simplifyArray(res.data)
    }
  })
}
...
async function getTableData(isSerch) {
  var obj = {
    ...
    roleId: sysRoleSelectValue.value,
    pageIndex: currentPage.value,
    pageSize: currentPageSize.value
  }
  await getSysMenuRecord(obj).then((res) => {
    if (res.code == 200 && res.success) {
      //判断数据是否覆盖分页
      if (res.total > 0 && Math.ceil(res.total / currentPageSize.value) < currentPage.value) {
        message.warning('数据量过少，请调整分页大小！')
      } else {
...
async function delAll(resolve, reject) {
  await deleteAllApi(false).then((res) => {
    resolve()
    if (res.code === 200 && res.success) {
