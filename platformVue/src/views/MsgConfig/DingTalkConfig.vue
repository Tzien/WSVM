  ...
  <div class="DingTalkconfig">
    <div style="height: 40px; line-height: 40px; text-align: right">
      ...
      <div class="headerRight">
        <a-p>appKey：</a-p>
        <a-input style="height: 68%" v-model:value="appKey" placeholder="appKey" />
        <a-p>agentId：</a-p>
        <a-input style="height: 68%" v-model:value="agentId" placeholder="agentId" />
        <CustomButtonList
          :ParamsRoleId="inputRoleId"
          :ParamsFunctionName="inputFunctionName"
          :BtnFunctionNameArray="['A040304', 'A040302', 'A040306', 'A040301']"
      ...
      :scroll="{ y: 'calc(100vh - 96px - 40px - 55px - 40px - 35px)' }"
        ...
        >
          <template #bodyCell="{ text, column, record }">
            <template v-if="column.dataIndex === 'appSecret'">
              <div class="ellipsis">{{ record.appSecret }}</div>
            </template>
            <!-- 操作按钮 -->
            <template v-if="column.dataIndex === 'operation'">
              <a @click="detail(record.dingTalkConfigId)" v-if="allDyBtn.find((item) => item.functionCode === 'A040303')">
                <CustomButtonListTable
                  :ObjItem="allDyBtn.find((item) => item.functionCode === 'A040303')"
                  :IsOnlyIcon="true"
                  style="display: inline; margin-right: 10px"
                ></CustomButtonListTable>
              </a>
              <a @click="onDelete(record.dingTalkConfigId)" v-if="allDyBtn.find((item) => item.functionCode === 'A040305')">
                <CustomButtonListTable
                  :ObjItem="allDyBtn.find((item) => item.functionCode === 'A040305')"
                  :IsOnlyIcon="true"
                  style="display: inline; margin-right: 10px"
                ></CustomButtonListTable>
              </a>
...
import { saveApi, getAllApi, getDetailApi, deleteApi, setSelectApi } from '@/api/Msg/dingtalkconfig'
...
const inputFunctionName = ref('A0403')
...
onMounted(() => {
  getAll(current.value, pageSize.value, appKey.value, agentId.value)
  getAllButton()
})
...
const appKey = ref()
const agentId = ref()
async function getAll(page, size, appKey, agentId) {
  const query = {
    pageIndex: page,
    pageSize: size,
    appKey: appKey,
...
const onShowSizeChange = (current, pageSize) => {
  console.info(1)
  getAll(current, pageSize, appKey.value, agentId.value)
}
const pageSizeChange = (page, pageSize) => {
  console.info(2)
  getAll(page, pageSize, appKey.value, agentId.value)
}
const columns = [
  {
    title: '序号',
...
async function detail(id) {
  ...
  if (response.code === 200 && response.success) {
    formState.dingTalkConfigId = response.data.dingTalkConfigId
    formState.appKey = response.data.appKey
    formState.appSecret = response.data.appSecret
...
async function setSelect() {
  ...
  const params = {
    id: rowSelection.value.selectedRow[0].dingTalkConfigId
  }
