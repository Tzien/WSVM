    ...
    public class FormDbController : ControllerBase
    {
        ...
        public async Task<dynamic> Uploader(string type,ChunkModel input)
        {
            ...
            if (AllowImageType(fileType) && type.Equals("annexpic"))
            {
                ...
                return new FileControlsModel { name = input.fileName, url = string.Format("/api/FormDb/Image/{0}/{1}", type, input.fileName), thumbUrl = string.Format("/api/File/Image/{0}/{1}", type, input.slImgName), fileExtension = fileType, fileSize = input.file.Length, fileName = input.slImgName };
            }
            else
            {
