...
public sealed partial class StringLoggingPart
{
    ...
    internal (ILogger, ILoggerFactory, bool) GetLogger()
    {
        ...
        else
        {
            try
            {
                logger = App.App.GetRequiredService(typeof(ILogger<>).MakeGenericType(categoryType)) as ILogger;
            }
            catch
            {
                hasException = true;
    ...
    {
        var consoleFormatterExtendOptions = App.App.GetOptions<ConsoleFormatterExtendOptions>();
