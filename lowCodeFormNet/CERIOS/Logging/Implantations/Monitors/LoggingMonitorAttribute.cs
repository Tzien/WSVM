...
using CERIOS.JsonSerialization.Extensions;
using CERIOS.Logging;
using CERIOS.Templates;
using CERIOS.UnifyResult;
...
public sealed class LoggingMonitorAttribute : Attribute, IAsyncActionFilter, IAsyncPageFilter, IOrderedFilter
{
    ...
    private async Task MonitorAsync(MethodInfo actionMethod, IDictionary<string, object> parameterValues, FilterContext context, dynamic next)
    {
        ...
        // 获取最终写入日志消息格式
        var finalMessage = GetJsonBehavior(JsonBehavior, monitorMethod) == CERIOS.Logging.JsonBehavior.OnlyJson ? jsonString : monitorMessage;
