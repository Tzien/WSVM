...
using CERIOS.ClayObject;
using CERIOS.ClayObject.Extensions;
using CERIOS.DataValidation;
using CERIOS.FriendlyException;
using CERIOS.JsonSerialization;
using CERIOS.Templates.Extensions;
using CERIOS.VirtualFileServer;
...
namespace CERIOS.RemoteRequest;
...
public sealed partial class HttpRequestPart
{
    ...
    public async Task<T> SendAsAsync<T>(CancellationToken cancellationToken = default)
    {
        ...
        // 解析 Json 序列化提供器
        var jsonSerializer = App.App.GetService(JsonSerializerProvider, RequestScoped ?? App.App.RootServices) as IJsonSerializerProvider;
    ...
    public async Task<HttpResponseMessage> SendAsync(CancellationToken cancellationToken = default)
    {
        ...
        // 创建客户端请求工厂
        var clientFactory = App.App.GetService<IHttpClientFactory>(RequestScoped ?? App.App.RootServices)
        ...
        var logger = App.App.GetService<ILogger<RemoteRequestService>>();
        ...
        {
            // 打印成功请求
            App.App.PrintToMiniProfiler(MiniProfilerCategory, "Succeeded", $"[StatusCode: {response.StatusCode}] Succeeded");
        ...
        else
        {
            ...
            // 打印失败请求
            App.App.PrintToMiniProfiler(MiniProfilerCategory, "Failed", $"[StatusCode: {statusCode}] {errors}", exception != null);
    ...
    private HttpRequestMessage CreateHttpRequestMessage(HttpClient httpClient, string httpClientOriginalString)
    {
        ...
        // 打印发送请求
        App.App.PrintToMiniProfiler(MiniProfilerCategory, "Sending", $"[{Method}] {httpClientOriginalString}{request.RequestUri?.OriginalString}");
